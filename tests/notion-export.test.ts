...
describe('Notion learning export', () => {
  it('creates a child page using structured blocks', async () => {
    ...
    await expect(exportLearningToNotion('2026-07-12', [learning])).resolves.toBe('https://notion.so/test-page');
    const [, request] = fetchMock.mock.calls[0] as [string, RequestInit];
    expect(request.headers).toMatchObject({'Notion-Version': '2026-03-11'});
    expect(String(request.body)).toContain('Founder Learning — 2026-07-12');
